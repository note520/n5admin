...
class Category_m extends CI_Model{
	...
	//取所有数据
	public function get_all_data($bool=true){
	...
	//当前位置类别
	public function location_sort($cid){
		$array = array();
		do{
			$query=$this->db->query("SELECT cid,pid,cname FROM my_categories WHERE cid='$cid'");
			$temp=$query->row_array(1);
			$array[]=$temp;
			$cid=$temp['pid'];
		}while($temp['pid']!=0);
		/*
